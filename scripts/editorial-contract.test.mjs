...
test('publication validates before replacing the previous manifest', () => {
  ...
  writeFileSync(join(dir,'prompts/published-articles.json'),JSON.stringify({version:1,articles:[]}));
  const file = join(dir, 'boletim-urologia-2026-09-13.html');
  writeFileSync(file, html.replace('data-source-id="1"','data-source-id="1" data-subramo="urologia-geral"'));
  ...
  const ledgerBefore=readFileSync(join(dir,'prompts/published-articles.json'),'utf8');
  const repeat=join(dir,'boletim-urologia-2026-09-14.html');
  writeFileSync(repeat,html.replaceAll('2026-09-13','2026-09-14').replace('data-source-id="1"','data-source-id="1" data-subramo="urologia-geral"'));
  writeFileSync(join(dir,'evidencias/material-urologia-2026-09-14.md'),material);
  const duplicated=spawnSync(process.execPath,[script,dir],{encoding:'utf8'});
  assert.equal(duplicated.status,1);
  assert.match(duplicated.stderr,/já publicado/);
  assert.equal(readFileSync(join(dir,'feed.json'),'utf8'),before);
  assert.equal(readFileSync(join(dir,'prompts/published-articles.json'),'utf8'),ledgerBefore);
  unlinkSync(repeat);
